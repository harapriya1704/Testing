...
from logger import logger  # Import logger for error handling
...
class LLMAnalyser:
    ...
    def _load_filter_prompt_template(self):
        """Prompt template for filtering Splunk events"""
        return """**Splunk Event Filtering**
        
        **Task**: Analyze these Splunk events and return ONLY entries that indicate potential customer experience issues.
        Return nothing if no issues found. Focus on:
        ...
        **Output Format**:
        Return ONLY the problematic events in this format:
        [Time] [Status] [URL] [Response Time]
        [Additional context if available]
        ---
        """
        
    def _load_prompt_template(self):
        """Updated prompt template with Splunk data source"""
    ...
    **Data Sources**:
    ...
    6. Filtered Splunk Events: {filtered_splunk}
    ...
    **Analysis Instructions**:
    ...
    5. Pay special attention to Splunk events indicating issues
    6. If multiple causes: List all relevant
    ...
    **Output Format**:
    ...
    def filter_splunk_events(self, events):
        ...
        try:
            if not events or "No events found" in events:
                return "No relevant Splunk events"
                
            formatted_prompt = self.filter_prompt_template.format(
                events=events
            )
    ...
    def _extract_page_names(self, server_sessions):
        ...
        if "No error sessions found" in server_sessions or not server_sessions:
    ...
    def analyze_dsat(self, context):
        """Analyze DSAT using all available data including filtered Splunk events"""
        try:
            # Preprocess server sessions
            server_pages = self._extract_page_names(context.get("Server-Sessions", ""))
            
            # Filter Splunk events
            splunk_events = context.get("Splunk Events", "")
            filtered_splunk = self.filter_splunk_events(splunk_events)
            
            # Prepare prompt with all data sources
            formatted_prompt = self.prompt_template.format(
                ...
                filtered_splunk=filtered_splunk
            )
